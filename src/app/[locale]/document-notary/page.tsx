...
import DocumentNotary from '@/components/DocumentNotary';
...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    alternates: buildAlternates(locale, '/document-notary'),
    openGraph: buildOpenGraph({ locale, path: '/document-notary', title, description }),
  };
}

export default async function DocumentNotaryPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations('Notary');
